...
namespace TeamDinnerOD
{
    public partial class VT : Form
    {
        ...
        private void VT_Load(object sender, EventArgs e)
        {
            ...
            //下拉選單(部門)
            Department.Items.Add("3M/C"); Department.Items.Add("5M/C"); Department.Items.Add("6M/C");
            Department.Items.Add("7M/C"); Department.Items.Add("A區"); Department.Items.Add("B區");
            Department.Items.Add("C區"); Department.Items.Add("D區"); Department.Items.Add("ISO中心");
            Department.Items.Add("PM3B"); Department.Items.Add("RD研發"); Department.Items.Add("土建監工");
            Department.Items.Add("文件中心"); Department.Items.Add("水環保課"); Department.Items.Add("出差人員");
            Department.Items.Add("出差主管"); Department.Items.Add("安管"); Department.Items.Add("安管守衛");
            Department.Items.Add("品管課"); Department.Items.Add("倉檢課"); Department.Items.Add("財會");
            Department.Items.Add("配送交貨"); Department.Items.Add("配送成品"); Department.Items.Add("控源");
            Department.Items.Add("排抄"); Department.Items.Add("統購"); Department.Items.Add("統購中心");
            Department.Items.Add("減廢"); Department.Items.Add("新進"); Department.Items.Add("業務");
            Department.Items.Add("業務客服"); Department.Items.Add("資訊科技"); Department.Items.Add("電儀電氣");
            Department.Items.Add("電儀儀錶"); Department.Items.Add("預保"); Department.Items.Add("製程技術");
            Department.Items.Add("製漿課"); Department.Items.Add("廠務管理"); Department.Items.Add("熱電");
            Department.Items.Add("總務HR人"); Department.Items.Add("總務HR事"); Department.Items.Add("總務來賓");
            Department.Items.Add("總務外籍"); Department.Items.Add("總務餐飲"); Department.Items.Add("總處資訊");
            Department.Items.Add("購料"); Department.Items.Add("擴建專案");
        ...
        private void ClearInfomation()
        {
            ...
            txtCardNo.Text = "";
            btnVTCheck.Enabled = true;
        }

        // 檢查鍵
        private void btnVTCheck_Click(object sender, EventArgs e)
        {
            // 取得使用者輸入的卡號和日期範圍
            ...
            using (SqlConnection connection = DatabaseHelper.GetConnection())  // 假設 DatabaseHelper 是你管理資料庫連線的類別
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            MessageBox.Show("該卡號為配送團體卡!\n請使用團體系統!", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            ClearInfomation();
                            return;
                        }
                        else
        ...
        private void btnAddVT_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                using (SqlConnection sqc = DatabaseHelper.GetConnection())
                {
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i <= alldays; i++)
                                {
                                    ...
                                    if (txtCardNo.Text == "0257964541")
                                    {
                                        ...
                                        MessageBox.Show($"卡號 : {txtCardNo.Text}\n是團體配送專用卡!!!", "新增失敗", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                        return;
                                    }
                                    else
                                    {
                                        // 當卡號不是 "0257964541" 時執行以下方法
                                        InsertBookDinnerData(sqc, transaction, idtmp, startDate, Dtype);
                    ...
                    {
                        btnAddVT.Enabled = true;  // 確保完成後重新啟用按鈕
                    }
                }
            }
        }
        ...
        private void InsertBookDinnerData(SqlConnection sqc, SqlTransaction transaction, string idtmp, DateTime startDate, int Dtype)
        {
            SqlCommand cmd = new SqlCommand("INSERT INTO BookDinner (ID, DDate, BeVisit, Class, Department, Reason, CardNo, DType, Md1, Md2, Memo, dbs, Gp, Num2, Num) " +
                                            "VALUES (@Id, @Date, @Name, '', @Dept, @Reason, @CardNo, @DinnerType, '2', '2', '', 'T4', 0, 1, @CardNoAgain)", sqc, transaction);

            cmd.Parameters.AddWithValue("@Id", idtmp);
            cmd.Parameters.AddWithValue("@Date", startDate.ToString("yyyy-MM-dd"));
            cmd.Parameters.AddWithValue("@Name", txtName.Text);
            cmd.Parameters.AddWithValue("@Dept", Department.Text + "-來賓");
        ...
        private void InsertBaseBookData(SqlConnection sqc, SqlTransaction transaction, string idtmp, DateTime startDate, int Dtype)
        {
            SqlCommand baseBookCmd = new SqlCommand("INSERT INTO BaseBook (ID, BDate, DType, CardNo, UserID, Name, Department, Flag, BNum, DNum, DinnerArea, Expense, dbs, chkup) " +
                                                    "VALUES (@Id, @Date, @DinnerType, @CardNo, '55555', @Reason, @Dept, 5, 1, 0, 2, 0, 'T4', 0)", sqc, transaction);
            ...
            baseBookCmd.Parameters.AddWithValue("@Dept", Department.Text + "-來賓");
        ...
        private void InsertBookDinnerLogData(SqlConnection sqc, SqlTransaction transaction, string idtmp, DateTime startDate, int Dtype)
        {
            SqlCommand BDLogCmd = new SqlCommand("INSERT INTO [db_owner].[BookDinnerLog] (loguser, logtime, logapps, username, dbs, Dtype, CardNo, changeaddease, changedate, Num2) " +
                                                 "VALUES (@UserID, @LogDate,'BookVisiter1', @Reason, 'T4', @DinnerType, @CardNo, 1, @Date, 1)", sqc, transaction);
